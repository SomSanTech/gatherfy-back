...
class UserController(
    ...
    fun getProfile(@RequestHeader("Authorization") token: String): User? {
        return userService.getUserProfile(tokenService.getSubjectFromToken(token.substringAfter("Bearer ")).toLong())
    }

    @GetMapping("/v2/profile")
    fun getProfileWithSocials(@RequestHeader("Authorization") token: String): ProfileDTO {
        return userService.getUserProfileWithSocials(tokenService.getSubjectFromToken(token.substringAfter("Bearer ")).toLong())
    }
    ...
    fun editProfile(@RequestHeader("Authorization") token: String,@Valid @RequestBody userEdit: EditUserDTO): User? {
        return userService.updateUser(tokenService.getSubjectFromToken(token.substringAfter("Bearer ")),userEdit)
    }
    ...
    fun editPassword(@RequestHeader("Authorization") token: String,@RequestBody editPasswordDTO: EditPasswordDTO): ResponseEntity<String> {
        return userService.updatePassword(tokenService.getSubjectFromToken(token.substringAfter("Bearer ")),editPasswordDTO)
    }
}